...
from zoo_attack import zgla_algorithm
...
def parse_args():
    ...
    parser.add_argument("--algorithm", help="choose zgla, fgla, dlg, stg, ig to use", default="zgla")
    ...
    parser.add_argument("--num_pert", help="numbers of perturbations for zeroth-order optimization", default=12, type=int)
...
if __name__ == "__main__":
    ...
    if args.algorithm not in ["fgla", "dlg", "stg", "ig", "zgla"]:
    ...
    usegz = True
    if args.algorithm == "zgla":
        usegz = False
    grad_dl = get_grad_dl(resnet50, dataloader, device=args.device,
                          seed=seed, num_pert=args.num_pert, usegz=usegz)
